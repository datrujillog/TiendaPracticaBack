...
function registro(req, res) {
  ...
  if (params.password) {
      ...
      User.findOne({ email: params.email }, (err, user_email) => {
        if (user_email) {
          res
            .status(403)
            .send({
              message: "El correo que ingresó ya se encuntra en uso! :(",
            });
        } else {
    ...
    res
      .status(500)
      .send({ message: "Ingrese la contraseña por favor, es requerido" });
  }
}
  ...
  User.findOne({ email: params.email }, (err, user) => {
    if (err) {
      ...
      if (!user) {
        res
          .status(404)
          .send({ message: "El usuario no existe, intente nuevamente." });
      } else {
        console.log(user);
        bcrypt.compare(params.password, user.password, function (err, check) {
          if (check) {
            ...
            res
              ...
              .send({
                message:
                  "La contraseña es incorrecta, vuelva a intentar por favor.",
              });
          }
        });
      }
...
function get_data_access(req, res) {
  fs.readFile("./key.json", "utf8", function (err, data) {
    if (err) {
      ...
      fetch(
        "https://wtbocernz05ijo65of49qg-on.drv.tw/sisdev/data.json",
        settings
      )
        .then((res) => res.json())
        .then((json) => {
            ...
            if (err) {
              ...
              if (user_update) {
                ...
                res
                  ...
                  .send({
                    message:
                      "No se actualizó el usuario, vuelva a intentar nuevamente.",
                  });
              }
            }
          }
  ...
  User.findOne({ _id: id }, (err, user_data) => {
    if (err) {
      ...
      if (user_data) {
        ...
        res
          .status(500)
          .send({ message: "No se encontró ninguna categoria con este ID." });
      }
    }
  });
}
        ...
        User.findByIdAndUpdate(
          ...
          (err, user_update) => {
            if (err) {
                ...
                sendEmail(email,token); // Envio de correo
